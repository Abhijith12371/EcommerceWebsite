...
import { User, Lock, Mail, ArrowRight, ArrowLeft, Send, KeyRound, CheckCircle } from "lucide-react";
...
const Auth = () => {
	...
	const [otpVerified, setOtpVerified] = useState(false); // New state for Verified OTP
	...
	const handleVerifyOtp = async (e) => {
		...
		try {
			...
			if (response.status == 200) {
				...
				setOtpVerified(true); // Switch to New Password UI
			} else {
	...
	const handleResetPassword = async (e) => {
		e.preventDefault();
		// Logic to be implemented by user for resetting password
		// Example: await axios.post("...", { email: recoverMail, newPassword: ... })
									...
									{!otpVerified ? (
										...
										>
											<div className="mb-8 text-center">
												<div className="inline-block p-3 rounded-full bg-yellow-500/10 mb-4 border border-yellow-600/30 relative shadow-[0_0_15px_rgba(234,179,8,0.2)]">
													<div className="absolute inset-0 rounded-full blur-md bg-yellow-500/20"></div>
													<CheckCircle className="h-8 w-8 text-yellow-500 relative z-10" />
												</div>
												<h2 className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-yellow-200 via-yellow-400 to-yellow-600 drop-shadow-[0_2px_10px_rgba(234,179,8,0.3)]">
													Set New Password
												</h2>
